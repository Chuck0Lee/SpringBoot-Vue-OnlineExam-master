  ...
  <el-table :data="pagination.records" border>
    ...
    <el-table-column fixed="right" label="操作" width="150">
      <template slot-scope="scope">
        <el-button @click="add(scope.row.examCode,scope.row.source)" type="primary" size="small">简答题打分</el-button>
      </template>
    </el-table-column>
  </el-table>
  <el-pagination
    ...
    export default {
      ...
      methods: {
        getExamInfo() { //分页查询所有试卷信息
          ...
          this.$axios(`/api/exams/${this.pagination.current}/${this.pagination.size}/${teacherId}`).then(res => {
        ...
        add(examCode,source) { //简答题情况
          this.$router.push({path:'/answerScore',query: {examCode: examCode,subject:source}})
        }
      },
    }
</script>
