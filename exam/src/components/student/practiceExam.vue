  ...
  export default {
    ...
    methods: {
      ...
      getExamInfo() {
        ...
        this.$axios(`/api/subjectP/${this.pagination.current}/${this.pagination.size}/${grade}/${major}`).then(res => {
      ...
      search() {
        ...
        let grade = this.$cookies.get("cgrade")
        let major = unescape(this.$cookies.get("cmajor"))
        this.$axios(`/api/subjectPs/${grade}/${major}`).then(res => {
      ...
      //跳转到试卷详情页
      toExamMsg(subject) {
        this.$router.push({path: '/startExam', query: {subject: subject}})
        console.log(subject)
      }
    }
  }
