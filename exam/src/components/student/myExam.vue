...
export default {
  ...
  methods: {
    ...
    getExamInfo() {
      ...
      let subject = this.$route.query.subject //获取路由传递过来的课程名称
      // console.log(grade)
      this.$axios(`/api/exams/${this.pagination.current}/${this.pagination.size}/${grade}/${major}/${subject}`).then(res => {
    ...
    search() {
      ...
      this.$axios(`/api/examss/${grade}/${major}`).then(res => {
