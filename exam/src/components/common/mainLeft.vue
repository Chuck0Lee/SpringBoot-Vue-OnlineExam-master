...
<template>
  <div id="left">
    <el-menu
      ...
      <el-submenu v-for="(item,index) in menu" :index='item.index' :key="index" v-if="role==1">
        <template slot="title">
    ...
    <!--<el-menu-->
      ...
      <!--background-color="#242f42"-->
      <!--menu-trigger="click" router>-->
      <el-submenu v-for="(item,index) in menu1" :index='item.index' :key="index" v-if="role==0">
        <template slot="title">
          <div class="left-width">
            <i class="iconfont" :class="item.icon"></i>
            <span slot="title" class="title">{{item.title}}</span>
...
export default {
  ...
  computed: mapState(["flag","menu","menu1"]),
  // computed: mapState(["flag1","menu1"]),
  created() {
    // this.addData()
